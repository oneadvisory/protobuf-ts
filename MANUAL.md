...
The generated code has no dependencies besides the runtime (@oneadvisory/protobuf-ts-runtime) and
strictly conforms to the protobuf spec.

The available packages are listed [here](./packages/) - but you probably want to
start with the plugin `@oneadvisory/protobuf-ts-plugin`.
...
# with yarn:
yarn add @oneadvisory/protobuf-ts-plugin
```
...
- "long_type_string"  
  ...
  Only applies to fields that do _not_ use the option `jstype`.
...
- "client_none"  
  ...
  Only applies to services that do _not_ use the option `ts.client`.
...
- "server_none"  
  ...
  _not_ use the option `ts.server`.
...
- "server_generic"  
  ...
  Only applies to services that do _not_ use the option `ts.server`.
...
The prefix "FOO\_" and "MOO_FOO" are dropped in TypeScript (unless `keep_enum_prefix` option is provided to the plugin):
...
```typescript
Any.toJson(any, {
  typeRegistry: [MyMessage, MyOtherMessage],
});
```
...
let rule: JsonValue | undefined = readMethodOption(
  AnnotatedService,
  'get',
  'google.api.http'
);
...
const nodeBinaryReadOptions = {
  readerFactory: (bytes: Uint8Array) =>
...
with a message and _then_ sends an error status. If you do not check the
