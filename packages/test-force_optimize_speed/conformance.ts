...
import {
  ...
  TestCategory,
  WireFormat,
} from './gen/conformance/conformance';
...
function doTest(
  ...
  const testMessageType = typeRegistry.find(
    (mt) => request.messageType === mt.typeName
  );
...
function doTestIo(
  testFn: (request: ConformanceRequest) => ConformanceResponse
): boolean {
