 ...
 *   messages are queued.
 */
export interface RpcOutputStream<T extends object = object>
...
type NextCallback<T extends object> = (
  message: T | undefined,
  ...
  implements RpcOutputStream<T>
{
  ...
  notifyNext(
    ...
    assert(
      (message ? 1 : 0) + (error ? 1 : 0) + (complete ? 1 : 0) <= 1,
