...
export class TestTransport implements RpcTransport {
   ...
   */
  private async streamResponses<I extends object, O extends object>(
    method: MethodInfo<I, O>,
    stream: RpcOutputStreamController<O>,
    abort?: AbortSignal
  ): Promise<void> {
  ...
  unary<I extends object, O extends object>(
    ...
    const requestHeaders = options.meta ?? {},
      headersPromise = this.promiseHeaders().then(
        delay(this.headerDelay, options.abort)
      ),
...
class TestInputStream<T> implements RpcInputStream<T> {
  ...
  private readonly data: Pick<
    TestTransportMockData,
    'inputMessage' | 'inputComplete'
  >;
  private readonly abort?: AbortSignal;

  constructor(
    data: Pick<TestTransportMockData, 'inputMessage' | 'inputComplete'>,
    abort?: AbortSignal
  ) {
    this.data = data;
    this.abort = abort;
  ...
  send(message: T): Promise<void> {
    ...
    const delayMs =
      this.data.inputMessage === undefined ? 10 : this.data.inputMessage;
  ...
  complete(): Promise<void> {
    ...
    const delayMs =
      this.data.inputComplete === undefined ? 10 : this.data.inputComplete;
    return Promise.resolve(undefined)
      .then(() => {
