...
describe('generated code compatibility', () => {
  ...
  it('should have same serialization order regardless of optimization options', function () {
    const reflectionType = new MessageType<TestAllTypesProto3>(
      TestAllTypesProto3.typeName,
      [...TestAllTypesProto3.fields].reverse()
    );
    ...
    expect(base64encode(TestAllTypesProto3.toBinary(message))).toBe(
      base64encode(reflectionType.toBinary(message))
    );
  });
});
