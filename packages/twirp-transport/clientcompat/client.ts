...
export async function client() {
  ...
  log(
    'client start with message: ' +
      ClientCompatMessage.toJsonString(message, { emitDefaultValues: true })
  );

  let client = new CompatServiceClient(
...
async function doNoop(client: ICompatServiceClient, req: Uint8Array) {
  ...
  log(
    'request (Empty) as json: ' +
      Empty.toJsonString(request, { emitDefaultValues: true })
  );
  ...
  try {
    ...
    log(
      'response (Empty) as json: ' +
        Empty.toJsonString(response, { emitDefaultValues: true })
    );
    log(
      'response (Empty) as protobuf: ' + byteArrToStr(Empty.toBinary(response))
    );
...
async function doMethod(client: ICompatServiceClient, req: Uint8Array) {
  ...
  log(
    'request (Req) as json: ' +
      Req.toJsonString(request, { emitDefaultValues: true })
  );
  ...
  try {
    ...
    log(
      'response (Resp) as json: ' +
        Resp.toJsonString(response, { emitDefaultValues: true })
    );
    log(
      'response (Resp) as protobuf: ' + byteArrToStr(Resp.toBinary(response))
    );
