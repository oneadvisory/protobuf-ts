...
export class InternalBinaryWrite implements CustomMethodGenerator {
  ...
  make(
    ...
    let internalBinaryWrite = this.makeMethod(source, descMessage, [
  ...
  private makeMethod(
    ...
    const MessageInterface = this.imports.type(source, descMessage),
      IBinaryWriter = this.imports.name(
        source,
        'IBinaryWriter',
    ...
    return ts.factory.createMethodDeclaration(
      ...
      [
        ...
        ts.factory.createParameterDeclaration(
          ...
          undefined,
          ts.factory.createTypeReferenceNode(BinaryWriteOptions, undefined)
        ),
      ],
      ts.factory.createTypeReferenceNode(IBinaryWriter, undefined),
  ...
  private makeStatementsForEveryField(
    ...
    for (let fieldInfo of interpreterType.fields
      .concat()
      .sort((a, b) => a.no - b.no)) {
      let descField = descMessage.fields.find(
      ...
      let fieldPropertyAccess = ts.factory.createPropertyAccessExpression(
        ts.factory.createIdentifier('message'),
        fieldInfo.localName
      );
      switch (fieldInfo.kind) {
        ...
        case 'enum':
          if (fieldInfo.repeat) {
            ...
            statements.push(
              ...this.scalarOneof(source, fieldInfo, fieldDeclarationComment)
            );
          } else {
            statements.push(
              ...this.scalar(
        ...
        case 'message':
          if (fieldInfo.repeat) {
            ...
            statements.push(
              ...this.messageOneof(source, fieldInfo, fieldDeclarationComment)
            );
          } else {
            statements.push(
              ...this.message(
  ...
  private scalar(
    ...
    fieldDeclarationComment: string
  ): ts.Statement[] {
    let type: rt.ScalarType =
    ...
    if (field.T === rt.ScalarType.BYTES && field.opt) {
      ...
      let defaultValue = new rt.MessageType<rt.UnknownMessage>(
        '$synthetic.InternalBinaryWrite',
        [field]
      ).create()[field.localName];
  ...
  private map(
    ...
    let mapEntryValueRead: ts.Expression =
    ...
    switch (field.K) {
      case rt.ScalarType.BOOL:
        ...
        mapEntryKeyRead = ts.factory.createBinaryExpression(
          ts.factory.createIdentifier('k'),
      ...
      case rt.ScalarType.SFIXED32:
        ...
        mapEntryKeyRead = ts.factory.createCallExpression(
    ...
    if (field.V.kind == 'message') {
      ...
      forBody = ts.factory.createBlock(
        [
          ...
          ts.factory.createExpressionStatement(
            this.makeWriterCall(
              ...
              this.makeWriterTagCall(
                ...
                1,
                this.wireTypeForSingleScalar(field.K)
              ),
          ...
          ts.factory.createExpressionStatement(
            this.makeWriterCall(this.makeWriterCall('writer', 'join'), 'join')
          ),
        ],
        true
      );
        ...
        this.makeWriterCall(
          this.makeWriterCall(
            this.makeWriterTagCall(
              ...
              2,
              this.wireTypeForSingleScalar(mapEntryValueScalarType)
            ),
  ...
  private makeWriterCall(
    writerExpressionOrName: string | ts.Expression,
    type: rt.ScalarType | 'fork' | 'join',
    argument?: ts.Expression
  ): ts.Expression {
    let methodName =
      typeof type == 'string' ? type : ScalarType[type].toLowerCase();
    let writerExpression =
      typeof writerExpressionOrName == 'string'
        ? ts.factory.createIdentifier(writerExpressionOrName)
        : writerExpressionOrName;
    let methodProp = ts.factory.createPropertyAccessExpression(
      writerExpression,
  ...
  private makeWriterTagCall(
    ...
    let wireTypeAccess = ts.factory.createPropertyAccessExpression(
      ts.factory.createIdentifier(
        this.imports.name(source, 'WireType', this.options.runtimeImportPath)
      ),
      wireTypeName
    );
