...
export class Interpreter {
  ...
  private readOptions(
    ...
    if (!type) {
      ...
      for (const desc of this.registry) {
        if (
          desc.kind == 'extension' &&
          desc.extendee.typeName === optionsSchema.typeName
        ) {
    ...
    const json = type.toJson(
      type.fromBinary(unknownBytes, { readUnknownField: false })
    );
    ...
    if (excludeOptions) {
      ...
      for (let key of Object.keys(json)) {
        for (let str of literals) if (key === str) delete json[key];
        for (let re of wildcards) if (key.match(re)) delete json[key];
      }
    }
   ...
   */
  getMessageType(
    descriptorOrTypeName: string | DescMessage
  ): rt.IMessageType<rt.UnknownMessage> {
    let descriptor =
  ...
  getEnumInfo(descriptorOrTypeName: string | DescEnum): rt.EnumInfo {
    const descriptor =
      typeof descriptorOrTypeName == 'string'
        ? this.registry.getEnum(descriptorOrTypeName)
        : descriptorOrTypeName;
    assert(descriptor);
    let enumInfo =
  ...
  private createTypescriptNameForField(
    descriptor: DescField | DescExtension | DescOneof,
    escapeCharacter = '$'
  ): string {
  ...
  private buildFieldInfos(
    fieldDescriptors: readonly (DescField | DescExtension)[]
  ): rt.PartialFieldInfo[] {
  ...
  private buildFieldInfo(
    fieldDescriptor: DescField | DescExtension
  ): undefined | rt.PartialFieldInfo {
    let info: { [k: string]: any } = {};
    ...
    if (
      fieldDescriptor.proto.jsonName !== rt.lowerCamelCase(fieldDescriptor.name)
    ) {
    ...
    if (fieldDescriptor.fieldKind == 'list') {
      info.repeat = fieldDescriptor.packed
        ? rt.RepeatType.PACKED
        : rt.RepeatType.UNPACKED;
    }

    // opt: Is the field optional?
    if (
      (fieldDescriptor.fieldKind == 'scalar' ||
        fieldDescriptor.fieldKind == 'enum') &&
      !fieldDescriptor.oneof
    ) {
      const proto3Optional = fieldDescriptor.proto.proto3Optional;
      const proto2Optional =
        fieldDescriptor.parent?.file.edition === Edition.EDITION_PROTO2 &&
        fieldDescriptor.proto.label === FieldDescriptorProto_Label.OPTIONAL;
    ...
    if (
      fieldDescriptor.fieldKind == 'scalar' ||
      (fieldDescriptor.fieldKind == 'list' &&
        fieldDescriptor.listKind == 'scalar')
    ) {
      // kind:
      info.kind = 'scalar';
      ...
      fieldDescriptor.fieldKind == 'message' ||
      (fieldDescriptor.fieldKind == 'list' &&
        fieldDescriptor.listKind == 'message')
    ) {
      // kind:
      info.kind = 'message';
  ...
  private findEnumSharedPrefix(
    ...
    let allNamesSharePrefix = names.every((name) =>
      name.startsWith(enumPrefix)
    );
    ...
    let strippedNamesAreValid = strippedNames.every(
      (name) => name.length > 0 && /^[A-Z].+/.test(name)
    );

    return allNamesSharePrefix && strippedNamesAreValid
      ? enumPrefix
      : undefined;
  }
  ...
  private getL(descField: DescField | DescExtension): rt.LongType | undefined {
    ...
    const jsTypeOption =
      descField.proto.options !== undefined &&
      isFieldSet(descField.proto.options, FieldOptionsSchema.field.jstype)
 ...
 * compatible with enums generated by @oneadvisory/protobuf-ts-plugin.
 */
export class RuntimeEnumBuilder {
  private readonly values: Array<{ name: string; number: number }> = [];
