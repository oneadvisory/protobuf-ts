...
import { ScalarType } from '@oneadvisory/protobuf-ts-runtime';
...
export class ProtobuftsPlugin extends PluginBaseProtobufES {
  ...
  generate(request: CodeGeneratorRequest): GeneratedFile[] {
    const options = parseOptions(
        request.parameter,
        `by protobuf-ts ${this.version}` +
          (request.parameter ? ` with parameter ${request.parameter}` : '')
      ),
      ...
      genMessageInterface = new MessageInterfaceGenerator(
        symbols,
        imports,
        comments,
        interpreter,
        ...
        switch (desc.kind) {
          ...
          case 'message':
            ...
            if (desc.typeName === 'google.protobuf.Timestamp') {
              ...
              symbols.register('Base64String', desc, outMain, 'Base64String');
            }
            break;
        }
      }
  ...
  /**
   * Helper to create a branded string type using type-fest's Tagged
   */
  private createBrandedStringType(
    ...
    // Create: export type TypeName = Tagged<string, 'TagName'>;
    const typeDecl = ts.factory.createTypeAliasDeclaration(
      [ts.factory.createModifier(ts.SyntaxKind.ExportKeyword)],
      ts.factory.createIdentifier(typeName),
      undefined,
      ts.factory.createTypeReferenceNode(
        ts.factory.createIdentifier('Tagged'),
        [
  ...
  protected transpile(tsFiles: OutFile[], options: Options): GeneratedFile[] {
    ...
    program.emit(
      ...
      (
        ...
        const tsFile = tsFiles.find(
          (x) => sourceFiles[0].fileName === x.getFilename()
        );
        if (!tsFile) {
          err = new Error(
  ...
  private isTypeUsed(
    type: DescMessage | DescEnum | DescExtension,
    ...
    for (const otherFile of files) {
      for (const otherType of nestedTypes(otherFile)) {
        if (otherType.kind === 'message') {
          if (
            otherType.fields.some(
              (descField) => descField.message?.typeName === type.typeName
            )
          ) {
            return true;
          }
          if (
            otherType.fields.some(
              (descField) => descField.enum?.typeName === type.typeName
            )
          ) {
            return true;
          }
...
export function createFileRegistryFromRequest(
