...
export class ServiceServerGeneratorGrpc {
  ...
  registerSymbols(source: TypescriptFile, descService: DescService): void {
    ...
    const definitionName = `${basename[0].toLowerCase()}${basename.substring(
      1
    )}Definition`;
  ...
  generateDefinition(source: TypescriptFile, descService: DescService) {
    const grpcServerDefinition = this.imports.type(
        source,
        descService,
        this.symbolKindDefinition
      ),
        ...
        [
          ts.factory.createVariableDeclaration(
            ...
            ts.factory.createObjectLiteralExpression(
              interpreterType.methods.map((mi) =>
                this.makeDefinitionProperty(source, mi)
              ),
              true
            )
          ),
