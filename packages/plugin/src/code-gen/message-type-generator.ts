...
export class MessageTypeGenerator {
  ...
  constructor(
    ...
    this.fieldInfoGenerator = new FieldInfoGenerator(
      this.registry,
      this.imports
    );
    this.wellKnown = new WellKnownTypes(this.imports, this.options);
    this.googleTypes = new GoogleTypes(this.imports, this.options);
    this.typeMethodCreate = new Create(
      this.imports,
      this.interpreter,
      this.options
    );
    this.typeMethodInternalBinaryRead = new InternalBinaryRead(
      this.registry,
      this.imports,
      this.interpreter,
      this.options
    );
    this.typeMethodInternalBinaryWrite = new InternalBinaryWrite(
      this.registry,
      this.imports,
      this.interpreter,
      this.options
  ...
  generateMessageType(
    ...
    const // identifier for the message
      MyMessage = this.imports.type(source, descMessage),
      Message$Type = ts.factory.createIdentifier(
        this.imports.type(source, descMessage) + '$Type'
      ),
      MessageType = ts.factory.createIdentifier(
        this.imports.name(source, 'MessageType', this.options.runtimeImportPath)
      ),
      interpreterType = this.interpreter.getMessageType(descMessage),
      classDecMembers: ts.ClassElement[] = [],
      classDecSuperArgs: ts.Expression[] = [
        // arguments to the MessageType CTOR
        // arg 0: type name
        ts.factory.createStringLiteral(descMessage.typeName),
        // arg 1: field infos
        this.fieldInfoGenerator.createFieldInfoLiterals(
          source,
          interpreterType.fields
        ),
      ];
    ...
    const classDec = ts.factory.createClassDeclaration(
      ...
      [
        ts.factory.createHeritageClause(ts.SyntaxKind.ExtendsKeyword, [
          ts.factory.createExpressionWithTypeArguments(MessageType, [
            ts.factory.createTypeReferenceNode(MyMessage, undefined),
          ]),
        ]),
      ],
      classDecMembers
    ...
    comment += this.comments
      .makeGeneratedTag(descMessage)
      .replace('@generated from ', '@generated MessageType for ');
