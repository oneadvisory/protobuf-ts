...
export class FieldInfoGenerator {
  ...
  createFieldInfoLiterals(
    source: TypescriptFile,
    fieldInfos: readonly rt.PartialFieldInfo[]
  ): ts.ArrayLiteralExpression {
    fieldInfos = fieldInfos.map((fi) =>
      FieldInfoGenerator.denormalizeFieldInfo(fi)
    );
    return ts.factory.createArrayLiteralExpression(
      fieldInfos.map((fi) => this.createFieldInfoLiteral(source, fi)),
      true
    );
  }

  createFieldInfoLiteral(
    source: TypescriptFile,
    fieldInfo: rt.PartialFieldInfo
      ...
      if (fieldInfo[key] !== undefined) {
        properties.push(
          ts.factory.createPropertyAssignment(
            key,
            typescriptLiteralFromValue(fieldInfo[key])
          )
        );
      }
    }
    ...
    if (fieldInfo.opt !== undefined) {
      properties.push(
        ts.factory.createPropertyAssignment(
          'opt',
          typescriptLiteralFromValue(fieldInfo.opt)
        )
      );
    }
    ...
    switch (fieldInfo.kind) {
      ...
      case 'enum':
        ...
        properties.push(
          ts.factory.createPropertyAssignment(
            ...
            this.createEnumT(source, fieldInfo.T())
          )
        );
        break;
      ...
      case 'message':
        ...
        properties.push(
          ts.factory.createPropertyAssignment(
            ts.factory.createIdentifier('T'),
            this.createMessageT(source, fieldInfo.T())
          )
        );
        break;
   ...
   */
  static createTypescriptLocalName(
    name: string,
    options: { useProtoFieldName: boolean }
  ): string {
  ...
  private createMessageT(
    ...
    return ts.factory.createArrowFunction(
      ...
      ts.factory.createIdentifier(generatedMessage)
    );
  }
  ...
  private createEnumT(
    ...
    return ts.factory.createArrowFunction(
      ...
      undefined,
      ts.factory.createToken(ts.SyntaxKind.EqualsGreaterThanToken),
      ts.factory.createArrayLiteralExpression(enumInfoLiteral, false)
    );
  }

  private createRepeatType(type: rt.RepeatType): ts.Expression {
    const expr = ts.factory.createNumericLiteral(type.toString());
    ts.addSyntheticTrailingComment(
      expr,
      ts.SyntaxKind.MultiLineCommentTrivia,
      `RepeatType.${rt.RepeatType[type]}`
    );
    return expr;
  }
  ...
  private createMapV(
    source: TypescriptFile,
    mapV: (rt.PartialFieldInfo & { kind: 'map' })['V']
  ): ts.ObjectLiteralExpression {
    let T: ts.Expression;
    ...
    switch (mapV.kind) {
      ...
      case 'scalar':
        ...
        if (mapV.L !== undefined) L = this.createLongType(mapV.L);
        break;
    }
    const properties: ts.ObjectLiteralElementLike[] = [
      ts.factory.createPropertyAssignment(
        ts.factory.createIdentifier('kind'),
        ts.factory.createStringLiteral(mapV.kind)
      ),
      ts.factory.createPropertyAssignment(ts.factory.createIdentifier('T'), T),
    ];
    if (L) {
      properties.push(
        ts.factory.createPropertyAssignment(ts.factory.createIdentifier('L'), L)
      );
    }
