...
export class ServiceTypeGenerator {
  ...
  registerSymbols(source: TypescriptFile, descService: DescService): void {
    this.symbols.register(
      createLocalTypeName(descService),
      descService,
      source
    );
  }
  ...
  generateServiceType(source: TypescriptFile, descService: DescService): void {
    const // identifier for the service
      MyService = this.imports.type(source, descService),
      ServiceType = this.imports.name(
        source,
        'ServiceType',
    ...
    const args: ts.Expression[] = [
      ts.factory.createStringLiteral(interpreterType.typeName),
      this.methodInfoGenerator.createMethodInfoLiterals(
        source,
        interpreterType.methods
      ),
    ];

    if (Object.keys(interpreterType.options).length) {
      args.push(typescriptLiteralFromValue(interpreterType.options));
    }
      ...
      ts.factory.createVariableDeclarationList(
        [
          ts.factory.createVariableDeclaration(
            ts.factory.createIdentifier(MyService),
            undefined,
            undefined,
    ...
    comment += this.comments
      .makeGeneratedTag(descService)
      .replace('@generated from ', '@generated ServiceType for ');
