...
const parseParameter = createOptionParser({
  ...
  server_generic: {
    ...
    description:
      ...
      'for example @oneadvisory/protobuf-ts-grpc-backend for gRPC. \n' +
...
export function parseOptions(
  ...
  const o: Writeable<Options> = {
    ...
    runtimeRpcImportPath: '@oneadvisory/protobuf-ts-runtime-rpc',
    runtimeImportPath: '@oneadvisory/protobuf-ts-runtime',
    ...
    getClientStyles(descriptor: DescService): ClientStyle[] {
      ...
      if (
        opt.includes(ClientStyle.NO_CLIENT) &&
        opt.some((s) => s !== ClientStyle.NO_CLIENT)
      ) {
        const err = new Error(
          `You provided invalid options for ${qualifiedName(
            descriptor
          )}. If you set (ts.client) = NO_CLIENT, you cannot set additional client styles.`
        );
    ...
    getServerStyles(descriptor: DescService): ServerStyle[] {
      ...
      // always check service options valid
      if (
        opt.includes(ServerStyle.NO_SERVER) &&
        opt.some((s) => s !== ServerStyle.NO_SERVER)
      ) {
        const err = new Error(
          `You provided invalid options for ${qualifiedName(
            descriptor
          )}. If you set (ts.server) = NO_SERVER, you cannot set additional server styles.`
        );
        err.name = `PluginMessageError`;
        throw err;
      }
