...
describe('interpreter', function () {
  it('recognizes field option jstype', function () {
    [rt.LongType.NUMBER, rt.LongType.STRING, rt.LongType.BIGINT].forEach(
      (normalLongType) => {
        ...
        expectFieldType(
          messageType,
          'fixed64_field_min_num',
          rt.LongType.NUMBER
        );
        expectFieldType(
          messageType,
          'fixed64_field_min_str',
          rt.LongType.STRING
        );
...
function expectFieldType(
  messageType: rt.IMessageType<rt.UnknownMessage>,
  name: string,
  type: rt.LongType
) {
