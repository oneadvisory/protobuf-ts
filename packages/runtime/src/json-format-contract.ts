 ...
 */
export function jsonReadOptions(
 ...
 */
export function jsonWriteOptions(
 ...
 */
export function mergeJsonOptions<
  T extends { typeRegistry?: readonly IMessageType<any>[] } & (
    | JsonWriteStringOptions
    | JsonReadOptions
  )
>(a?: Partial<T>, b?: Partial<T>): Partial<T> {
  const c = { ...a, ...b } as Partial<T>;
  (c as { typeRegistry?: readonly IMessageType<any>[] }).typeRegistry = [
    ...(a?.typeRegistry ?? []),
    ...(b?.typeRegistry ?? []),
  ];
  return c;
