...
import { adaptService } from '@oneadvisory/protobuf-ts-grpc-backend';
...
const exampleService: IExampleService = {
  ...
  async serverStream(
    request: ExampleRequest,
    responses: RpcInputStream<ExampleResponse>,
    context: ServerCallContext
  ): Promise<void> {
  ...
  async clientStream(
    requests: RpcOutputStream<ExampleRequest>,
    context: ServerCallContext
  ): Promise<ExampleResponse> {
  ...
  async bidi(
    ...
    await responses.send(
      ExampleResponse.create({
        answer:
          'you stopped asking, but I will send you one more message soon.',
      })
    );
    answersSent++;
